...
function outputResults(store, options, internalStore) {
  ...
  // handle internal broken links
  const brokenInternalLinks = internalStore
    ? internalStore.filter((link) => link.getExists() === false)
    : [];

  options.loggingLevel >= 1 &&
    brokenInternalLinks.forEach((link) => {
      const pages = link.getPages();
      log().error().display(`Internal link is broken: ${link.targetPath}`);
      log().display(`Used ${link.getLinkCount()} time(s) on these pages:`, 2);
      pages.forEach((page) => log().bullet().indent().display(page));
    });

  if (brokenInternalLinks.length > 0) debug(`found ${brokenInternalLinks.length} broken internal links`);

  if (options.internalLinks === "error" && brokenInternalLinks.length > 0)
    throw new Error("There are broken internal links in your build! See above for details.");
}
