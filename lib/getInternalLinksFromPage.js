...
function resolveTargetPath(href, outputPath, outputDir) {
  ...
  // Convert to a site-relative path using forward slashes regardless of platform
  // (path.resolve/path.sep produce backslashes on Windows).
  const siteRelative = resolved.slice(absOutputDir.length).split(path.sep).join("/");
